...
class Game3D:
    ...
    def draw_webcam_window(self, frame, gaze_x, gaze_y):
        ...
        if gaze_x is not None and gaze_y is not None:
            px, py = int(gaze_x * display_width), int(gaze_y * display_height)
            cv2.circle(frame_resized, (px, py), 8, (0, 255, 0), 2)
            cv2.circle(frame_resized, (px, py), 2, (0, 255, 0), -1)
        ...
        margin = 20
        x_pos = self.display[0] - display_width - margin
        y_pos = margin
        ...
        glVertex2f(x_pos - border_thickness, y_pos - border_thickness)
        glVertex2f(x_pos + display_width + border_thickness, y_pos - border_thickness)
        glVertex2f(x_pos + display_width + border_thickness, y_pos + display_height + border_thickness)
        glVertex2f(x_pos - border_thickness, y_pos + display_height + border_thickness)
        glEnd()
        
        # Draw webcam frame using glDrawPixels
        glRasterPos2i(x_pos, y_pos + display_height)
        glDrawPixels(display_width, display_height, GL_RGB, GL_UNSIGNED_BYTE, frame_rgb.tobytes())
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.show_webcam and ret:
                self.draw_webcam_window(frame, gaze_x, gaze_y)
...
if __name__ == "__main__":
    ...
    print("4. Webcam feed displays in top right corner")
    print("\nControls:")
